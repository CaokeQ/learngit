...
int  sendbuf(char *buf, int x, int y, SOCKET sclient, sockaddr_in sin, int len) {
	buf[0] = x + '0';
	buf[1] = y + '0';
	if (sendto(sclient, buf, 5, 0, (sockaddr*)&sin, len))
		return 0;
	return 1;
}

int receive(char *rbuf, SOCKET sclient, sockaddr_in sin, int len) {
	if (recvfrom(sclient, rbuf, 5, 0, (sockaddr*)&sin, &len) == SOCKET_ERROR)return 0;
	return 1;
}
...
*don't cls(simple)
*/
void chess_start(SOCKET socketClient,sockaddr_in sin)
{
	for (;;)
	{
		int x;
		...
		receive(rbuf, socketClient, sin, len);
		x = rbuf[0] - '0';
		y = rbuf[1] - '0';
		...
		if (chess_whowin() == BLACK)
		{
			...
			if (getchar() == 'y') {
				chessboard_init();
				memset(*chessboard, NONE, sizeof(*chessboard));
				continue;
			}
		...
		y = chess_Y();
		chessboard_DrawW(x, y);
		sendbuf(buf, x, y, socketClient,sin,len) ;
